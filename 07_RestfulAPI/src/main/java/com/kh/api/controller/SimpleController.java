 ...
 * - 하나의 URI는 하나의 고유한 리소스(Resoucr)를 대표하도록 설계하는 전송방식
 ...
 * - 메소드의 리턴 타입으로 사용자가 정의한 클래스 타입도 사용할 수 있고, 이 외에도 여러 타입으로 보내는데 이를 JSON이나 XML로 자동 처리
 * 
...
public class SimpleController {
	...
	@GetMapping("/greet") // jackson - core 추가해야 json 방식으로 객체 나옴
	public Person sayGreet() {
		Person p = new Person("김도경", "천재입니다");
		return p;
	}
	...
	public Map<Integer, Person> sendGreet() {
		...
		map.put(1, new Person("김도경", "정말 천재야!"));
		return map;
	}
}
